...
function Movie({ result }) {
  ...
  return (
    <div className='relative z-50'>
      ...
      <div className='absolute inset-y-28 md:inset-y-auto md:bottom-10 inset-x-4 md:inset-x-12 space-y-4 z-30 bg-black/80 py-3 px-3 md:px-6 max-h-[250px] md:max-w-[500px] md:max-h-[600px] lg:max-w-[800px]  rounded-md'>
        <h1 className='text-3xl md:text-5xl'>{result.name}</h1>
...
export async function getServerSideProps(context) {
  ...
  const request = await axios.get(`https://api.themoviedb.org/3/tv/${id}?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US&append_to_response=videos`).then((data) => (data));
